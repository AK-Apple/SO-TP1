...
#define RESULT_FILE_PATH_FORMAT "result/md5_result%d.txt"
...
ResultADT new_result_ADT(pid_t pid, int file_count) {
    ...
    if(result_ADT == NULL)
        ERROR_EXIT("malloc");
    char result_file_path[RESULT_PATH_SIZE] = {0};
    sprintf(result_file_path, RESULT_FILE_PATH_FORMAT, pid);
    ...
    sprintf(result_ADT->shared_memory_path, SHARED_MEMORY_PATH_FORMAT, pid);
    sprintf(result_ADT->mutex_semaphore_path, SEMAPHORE_MUTEX_PATH_FORMAT, pid);
    sprintf(result_ADT->full_buffer_semaphore_path, SEMAPHORE_BUFFER_PATH_FORMAT, pid);
...
int print_result(ResultADT result_ADT) {
    ...
    if(result_ADT->shared_memory_current_adress[0] != EOF) {
